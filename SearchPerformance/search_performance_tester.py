...
def linear_search_tester():
    ...
    # TODO: identify and implement additional test cases
    # ADD MORE TEST CASES

    # find a number at the end of the list
    result = linear_search(unsorted_list, 4)
    assert (type(result == int))
    assert (result == 3)

    # find a number at the beginning of the list
    result = linear_search(unsorted_list, 8)
    assert (type(result == int))
    assert (result == 0)

    # find a number much larger than any number in the list
    result = linear_search(unsorted_list, 15)
    assert (type(result == int))
    assert (result == -1)

    # find a number much smaller than any number in the list
    result = linear_search(unsorted_list, -30)
    assert (type(result == int))
    assert (result == -1)
    return True
...
def binary_search_tester():
    ...
    # find a number at the end of the list
    result = binary_search(sorted_list, 8)
    assert (type(result == int))
    assert(result == 3)

    # find a number that is much larger than any number in the list
    result = binary_search(sorted_list, 25)
    assert (type(result == int))
    assert(result == -1)

    # find a number that is much smaller than any number in the list
    result = binary_search(sorted_list, -25)
    assert (type(result == int))
    assert(result == -1)
...
def sorted_search_tester():
    ...
    # confirm the number of tuples
    assert(len(result) == 3)

    # confirm count and types of values in each tuple
    size = 2
